...
function CombuCreateTextureList()
    ...
    CombuTextureBorderFrame:SetPoint("CENTER",CombuTextureDropDown,"CENTER",0,0-(CombuTextureFrame:GetHeight()/4))
    CombuTextureBorderFrame:SetParent(CombuTextureDropDown)
    CombuTextureBorderFrame:SetFrameLevel(CombuTextureDropDownButton:GetFrameLevel()+1)
    ...
    for i = 1,#CombuTextureList do
        ...
        if (CombuTextureList[i]) == combusettingstable["combutexturename"] then
...
--Combustion/impact part
        ...
        if (b5 == nil) then
        elseif (b5<=2) and (combusettingstable["combureport"] == true) and (InCombatLockdown() == 1) then -- to show dot total damage in combat with report enabled
            CombustionUp = 1
            ImpactUp = 0
            combufadeout = false
            combuchatautohide = 0 
