...
local function CombustionIgnite(event, spellId, spellSchool, amount, critical, destGUID)
  ...
  if (
    ((critical == 1) and (event == "SPELL_DAMAGE")) and
    ((spellSchool == 4) or (spellSchool == 20)) and
    (spellId ~= 83853) and
    (spellId ~= 89091) and
    (spellId ~= 44461) and
    (spellId ~= 2120) and
    ...
    elseif (
      (combuignitetimer >= 1.7 - combusettingstable["combuignitedelta"]) and
      (combuignitetimer <= 2.5 + combusettingstable["combuignitedelta"])
    ) then
      combuignitetemp = combuignitetemp + combuigniteamount
    elseif (
      (combuignitetimer <= 0.5 + combusettingstable["combuignitedelta"]) and
      (combuignitetimer ~= 0)
    ) then
      combuignitetemp = combuignitetemp + combuigniteamount
  ...
  elseif (event == "SPELL_PERIODIC_DAMAGE") and (spellId == 12654) then
    ...
    -- color ignite text depending of accuracy of predicter
    if (combusettingstable["combuignitepredict"] == true) then
      if (tonumber(abs(combuigndamage - amount)) <= 3) then
        IgniteLabel:SetText(format(CombuLabel["dmggreen0"], amount))
      else
        IgniteLabel:SetText(format(CombuLabel["dmgred0"], amount))
      end
    ...
    combuignitetemp = 0
  elseif (
    (combucrittarget ~= UnitGUID("target")) or
    (UnitGUID("target") == nil) or
    ((event == "SPELL_AURA_REMOVED") and (spellId == 12654))
  ) then
  ...
  if (combuignitetimer >= combusettingstable["combutimervalue"]) and (combuignitetimer ~= 0) then
  ...
  elseif (combuignitetimer <= combusettingstable["combutimervalue"]) and (combuignitetimer ~= 0) then
    IgnTextFrameLabel:SetText(format("|cffff0000%.1f|r", combuignitetimer))
    IgniteButton:SetTexture("Interface\\AddOns\\CombustionHelper\\Images\\Combustionon")
    IgnTime = 0
  else
    IgnTextFrameLabel:SetText(format(CombuLabel["ignshort"]))
...
function Combustion_OnEvent(self, event, ...)
  if (event == "PLAYER_LOGIN") then
    ...
    if (combusettingstable["combuchat"] == true) then
      ChatFrame1:AddMessage(CombuLoc["startup"])
    end
    ...
    if (id1 == 61205) or (id4 == 61205) or (id6 == 61205) then
      ffbglyph = true
    else
      ffbglyph = false
    ...
    if (combusettingstable["combulock"] == false) then
      CombustionFrame:EnableMouse(true)
    elseif (combusettingstable["combulock"] == true) then
      CombustionFrame:EnableMouse(false)
    end
  ...
  if (event == "PLAYER_TALENT_UPDATE") then
    ...
    if (a6 == false) and (combutalent == true) then
      ...
      combutalent = false
      if (combusettingstable["combuchat"] == true) then
        ChatFrame1:AddMessage(CombuLoc["combuon"])
      end
    elseif (a6 == true) and (combutalent == false) then
      ...
      combutalent = true
      if (combusettingstable["combuchat"] == true) then
        ChatFrame1:AddMessage(CombuLoc["combuoff"])
      end
      CombustionFrameresize()
    end
  ...
  if (event == "GLYPH_ADDED") or (event == "GLYPH_REMOVED") then
    Combuffbglyphcheck()
  end
  ...
  if (event == "COMBAT_LOG_EVENT_UNFILTERED") then
    ...
    if (sourceName == UnitName("player")) then
      ...
      if (combusettingstable["combureport"] == true) and (destGUID == UnitGUID("target")) then
        ...
        if ((spellId == 44457) and (event == "SPELL_PERIODIC_DAMAGE")) then
          ...
          elseif (critical == 1) and (combumeta == false) then
            ...
            combulbdamage = amount
          end
          LBLabel:SetText(format(CombuLabel["dmgwhite0"], combulbdamage))

        -- pyroblast damage
        elseif (
          ((spellId == 11366) and (event == "SPELL_PERIODIC_DAMAGE")) or
          ((spellId == 92315) and (event == "SPELL_PERIODIC_DAMAGE"))
        ) then
          if (critical == 1) and (combumeta == true) then
      ...
      if (combusettingstable["comburefreshmode"] == true) and (spellId == 44457) then
        if (event == "SPELL_AURA_REFRESH") then
          combulbrefresh = combulbrefresh + 1
      ...
      if ((combusettingstable["combulbtracker"] == true) and (spellId == 44457)) then
        if (event == "SPELL_AURA_APPLIED") or (event == "SPELL_AURA_REFRESH") then
          CombuLBauratracker(destGUID, destName, GetTime())
        elseif (event == "SPELL_AURA_REMOVED") then
          for i = 1, #LBtrackertable do
            if LBtrackertable[i] and ((LBtrackertable[i])[1] == destGUID) then
              table.remove(LBtrackertable, i)
              break
            end
          end
      ...
      -----
      -- successful impact cast
      if (spellId == 2136) and (event == "SPELL_CAST_SUCCESS") then
        combuimpacttimer = GetTime()
      elseif (spellId == 2136) and (event == "SPELL_MISSED") then
        combuimpacttimer = nil
      end

      -------------------------------------------
      -- FlameStrike tracking
      if (
        (combusettingstable["combuflamestrike"] == true) or
        (spellId == 2120) and
        (spellId == 88148)
      ) then
        if (event == "SPELL_DAMAGE") or (event == "SPELL_CAST_SUCCESS") then
          ...
          elseif (spellId == 2120) then
            table.insert(LBtrackertable, {spellId, CombuLabel["flamestrike"], (GetTime() + 8), 8, nil})
          elseif (spellId == 88148) then
            table.insert(LBtrackertable, {spellId, CombuLabel["blastwave"], (GetTime() + 8), 8, nil})
          end
        end
      end
      ...
      if (combusettingstable["combureportpyro"] == true) then
        if (spellId == 48108) and (event == "SPELL_AURA_APPLIED") then
          combupyrogain = combupyrogain + 1
        elseif (spellId == 48108) and (event == "SPELL_AURA_REFRESH") then
          combupyrorefresh = combupyrorefresh + 1
          print(format(CombuLoc["pyrorefresh"]))
        elseif (
          ((spellId == 11366) and (event == "SPELL_CAST_SUCCESS")) or
          ((spellId == 92315) and (event == "SPELL_CAST_SUCCESS"))
        ) then
          combupyrocast = combupyrocast + 1
        end
      end
    ...
    end -- end `sourceName == UnitName("player")`

    -- if (combuhasteprocs[spellId] and destName == UnitName("player")) then
    --   print(event, spellId, spellName)
    --   CombuTickManager(event, spellId, spellName)
    -- end
  end -- end `event == "COMBAT_LOG_EVENT_UNFILTERED"`
  ...
  if (event == "PLAYER_REGEN_DISABLED") then
    ...
    if CombuCritMeta[gem1] or CombuCritMeta[gem2] or CombuCritMeta[gem3] then
      combumeta = true
    else
      combumeta = false
    end
  ...
  elseif (event == "PLAYER_REGEN_ENABLED") then
    if (
      (combusettingstable["combureportmunching"] == true) and
      (combuignitevalue ~= 0) and
      select(5, GetTalentInfo(2, 4)) ~= 0
    ) then
      print(
        format(
          CombuLoc["ignrep"],
          ...
          format(
            "%.0f / %.0f%%",
            combuignitevalue - combuigniteapplied,
            (combuignitevalue - combuigniteapplied) / combuignitevalue * 100
          )
        )
      )
    end
    ...
    if (combusettingstable["combureportpyro"] == true) and (combupyrogain ~= 0) then
      print(
        format(
          CombuLoc["pyroreport"],
          combupyrogain + combupyrorefresh,
          combupyrocast,
          (100 * (combupyrocast / (combupyrogain + combupyrorefresh)))
        )
      )
    end
    ...
    -- cleaning multiple LB tracker table when leaving combat
    table.wipe(LBtrackertable)
    IgniteLabel:SetText(format(CombuLabel["ignite"]))
  end -- end `event == "PLAYER_REGEN_ENABLED"`
end -- end Combustion_OnEvent
...
function Combustion_OnUpdate(self, elapsed)
  ...
  if (self.TimeSinceLastUpdate > Combustion_UpdateInterval) then
    ...
    elseif (combulbtimer <= combusettingstable["combutimervalue"]) and (combulbtimer ~= 0) then
      ...
      LBTextFrameLabel:SetText(format(CombuLabel["lbshort"])) 
    ...
    elseif (combuffbtimer <= combusettingstable["combutimervalue"]) and (combuffbtimer ~= 0) then
      ...
      FFBTextFrameLabel:SetText(format(CombuLabel["ffbshort"]))
    ...
    -- condition when timer is with less than 2 seconds left
    elseif (combupyrotimer <= combusettingstable["combutimervalue"]) and (combupyrotimer ~= 0) then
      PyroTextFrameLabel:SetText(format("|cffff0000%.1f|r",combupyrotimer))
      PyroButton:SetTexture("Interface\\AddOns\\CombustionHelper\\Images\\Combustionon")
      PyroTime = 0
    else
      PyroTextFrameLabel:SetText(format(CombuLabel["pyroshort"]))
    ...
    elseif (b5<=2) and (combusettingstable["combureport"] == true) and (InCombatLockdown() == 1) then 
      ...
      if (combusettingstable["combureportvalue"] <= combuigndamage) and combusettingstable["combureportthreshold"] then
        ...
        StatusTextFrameLabel:SetText(format(CombuLabel["ignCBred"], combuigndamage))
      end
    ...
    -- condition when impact is up and combustion in cd
    elseif (b5>=2) and (k7 == 64343) and (combusettingstable["combuimpact"] == true) then
    ...
    if (combusettingstable["combucrit"]==true) then
      ...
      elseif (k10==17800) then
        combucrittimer = (-1*(time-g10))
      else
        combucrittimer = 0
      end

      -- condition when timer is with more than 2 seconds left
      if (combucrittimer >= combusettingstable["combutimervalue"]) and (combucrittimer ~= 0) then
      ...
      elseif (combucrittimer <= combusettingstable["combutimervalue"]) and (combucrittimer ~= 0) then
        ...
        CritTextFrameLabel:SetText(format(CombuLabel["critmassred"]))
      ...
      elseif (k9==22959) then
        Critbar:Show()
        Critbar:SetValue((92+combucritwidth)*((g9-GetTime())/f9))
        Critbar:SetStatusBarColor(unpack(combusettingstable["barcolornormal"]))
      elseif (k10==17800) and (combucrittimer <= combusettingstable["combutimervalue"]) then
        Critbar:Show()
    ...
    end -- end Critical Mass/shadow mastery tracking
    ...
    if (combusettingstable["combureportthreshold"] == true) then
      ...
      if (
        (CombustionUp == 1) and
        (combusettingstable["combureportvalue"] <= combuigndamage) and
        (combusettingstable["combureportvalue"] ~= 0)
      ) then
      ...
      elseif (
        (CombustionUp == 0) and
        (ImpactUp == 1) and
        (combusettingstable["combureportvalue"] <= combuigndamage)
      ) then
        --yellow bg frame when threshold is met and impact are up
    ...
    --Green background for frame when dots and combustion are up
    elseif (
      (LBTime == 1) and
      (FFBTime == 1) and
      (IgnTime == 1) and
    ...
    if (
      (combusettingstable["combulbtracker"] == true) or
      (combusettingstable["combuflamestrike"] == true)
    ) then
      CombuLBtrackerUpdate()
    end
  ...
  end -- end large if block
end -- end Combustion_OnUpdate
...
SlashCmdList["COMBUCONFIG"] = function(msg)
  if (msg == "" or  msg == "help" or  msg == "?" or msg == "config") then
    ...
    if (combusettingstable["combuchat"]==true) then
      print(format(CombuLoc["slashcomm"]))
    end
  else
    InterfaceOptionsFrame_OpenToCategory("CombustionHelper")
    if (combusettingstable["combuchat"]==true) then
